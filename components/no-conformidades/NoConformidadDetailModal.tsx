...
import { exportNoConformidadPdf } from '@/components/no-conformidades/exportNoConformidadPdf';
...
export default function NoConformidadDetailModal({ nc, onClose, onUpdated }: Props) {
    ...
    const canActuate = session?.rol === nc.responsableRol;
    const imagenesActuales = nc.imagenes ?? [];
    const cuposDisponibles = Math.max(0, MAX_IMAGENES_NC - imagenesActuales.length - archivosAdjuntos.length);
    ...
    const handleAdjuntosChange = (event: ChangeEvent<HTMLInputElement>) => {
        ...
        if (cuposDisponibles <= 0) {
            setError(`Esta no conformidad ya tiene el máximo de ${MAX_IMAGENES_NC} imágenes.`);
            event.target.value = '';
            return;
        }
        ...
        if (archivosAAgregar.length < imagenesValidas.length) {
            setError(`Solo puedes adjuntar hasta ${MAX_IMAGENES_NC} imágenes en total.`);
        }
    ...
    const handleExportPdf = async () => {
        ...
        try {
            await exportNoConformidadPdf(nc);
        } catch (err) {
        ...
        try {
            ...
            if (archivosAdjuntos.length > 0) {
                ...
                const uploadResponse = await fetch('/api/upload', {
                    ...
                    body: JSON.stringify({
                        images: base64Images,
                        folder: 'no-conformidades/adjuntos',
                    }),
                });
            ...
            const res = await fetch(`/api/no-conformidades/${nc.id}`, {
                method: 'PATCH',
                headers: { 'Content-Type': 'application/json' },
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50">
            <div className="bg-white rounded-2xl shadow-2xl w-full max-w-2xl max-h-[90vh] flex flex-col">
                ...
                <div className="flex items-start justify-between p-6 border-b border-gray-200">
                    <div>
                        <h2 className="text-lg font-bold text-gray-900">No Conformidad #{nc.id}</h2>
                ...
                <div className="flex-1 overflow-y-auto p-6 space-y-6">
                    ...
                    <div className="bg-red-50 border border-red-200 rounded-xl p-4">
                        <p className="text-xs font-semibold text-red-500 uppercase tracking-wide mb-1">Ítem no conforme</p>
                ...
                {canActuate && nc.estado !== 'CERRADA' && (
                    <div className="border-t border-gray-200 p-6 space-y-3">
                        <textarea
                            ...
                            rows={2}
                            placeholder="Escribe un comentario sobre esta no conformidad..."
                        ...
                        <div className="space-y-2">
                            <div className="flex items-center justify-between">
                                <p className="text-xs text-gray-500">Adjuntar imágenes (máximo {MAX_IMAGENES_NC})</p>
                        ...
                        <div className="flex gap-2 flex-wrap">
                            ...
                            {/* Cerrar no conformidad — visible solo cuando hay comentario */}
                            {comentario.trim() && (
                                <button
                                ...
                                >
                                    {saving ? 'Guardando...' : 'Cerrar no conformidad'}
                                </button>
                            )}
                        </div>
                    </div>
