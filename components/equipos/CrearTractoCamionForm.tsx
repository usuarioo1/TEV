...
export default function CrearTractoCamionForm() {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            const response = await fetch('/api/equipos/tractocamiones', {
                method: 'POST',
                headers: {
            ...
            if (!response.ok) {
                ...
                throw new Error(data.error || 'Error al crear el tractocamión');
            }

            // Redirigir a la página de equipos
            router.push('/equipos?success=tractocamion');
            router.refresh();
        } catch (err) {
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <div>
                ...
                <p className="mt-1 text-sm text-gray-500">
                    Ingrese la patente del tractocamión (sin espacios ni guiones)
                </p>
            </div>
            ...
            <div className="flex gap-4">
                ...
                >
                    {isSubmitting ? 'Creando...' : 'Crear Tractocamión'}
                </button>
                <button
                    type="button"
                    onClick={() => router.back()}
