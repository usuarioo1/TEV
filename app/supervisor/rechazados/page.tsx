...
export default async function ServiciosRechazadosPage() {
    await requireRole([ROLES.SUPERVISOR, ROLES.JEFATURAS]);

    const session = await getSession();
    if (!session) redirect('/login');

    // Obtener servicios rechazados (por operario o por supervisor)
    const serviciosRechazados = await prisma.servicio.findMany({
        where: {
            OR: [
                { estado: 'RECHAZADO' }, // Rechazado por operario
                {
                ...
                <div className="mb-8">
                    <div className="bg-white shadow rounded-lg p-6 mb-6">
                        ...
                        <p className="text-sm text-gray-600">
                            Total: {serviciosRechazados.length} servicios
                        </p>
                    </div>
                    ...
                    {serviciosRechazados.length === 0 ? (
                        <div className="bg-white shadow rounded-lg p-8 text-center">
                            ...
                            <h3 className="mt-2 text-sm font-medium text-gray-900">No hay servicios rechazados</h3>
                            <p className="mt-1 text-sm text-gray-500">
                                No se han rechazado servicios
                            </p>
                        </div>
                    ) : (
                        <div className="space-y-4">
                            {serviciosRechazados.map((servicio) => {
                                const rechazadoPorOperario = servicio.estado === 'RECHAZADO';
                                const rechazadoPorSupervisor = servicio.aprobacion && !servicio.aprobacion.aprobado;

                                return (
                                    <div key={servicio.id} className="bg-white shadow rounded-lg overflow-hidden border-l-4 border-red-500">
                                        <div className="p-6">
                                            <div className="flex items-start justify-between mb-4">
                                                <div className="flex-1">
                                                    ...
                                                    <p className="text-sm text-gray-500 mt-1">
                                                        {rechazadoPorOperario && servicio.fechaRechazo && (
                                                            <>Rechazado por operario el {new Date(servicio.fechaRechazo).toLocaleDateString('es-ES', {
                                                                day: '2-digit',
                                                                month: 'long',
                                                                year: 'numeric',
                                                                hour: '2-digit',
                                                        ...
                                                        {rechazadoPorSupervisor && servicio.aprobacion?.fechaAprobacion && (
                                                            <>Rechazado por supervisor el {new Date(servicio.aprobacion.fechaAprobacion).toLocaleDateString('es-ES', {
                                                                day: '2-digit',
                                                                month: 'long',
                                                                year: 'numeric',
                                                                hour: '2-digit',
                                            ...
                                            {/* Motivo del rechazo */}
                                            {(servicio.motivoRechazo || servicio.aprobacion?.motivoRechazo) && (
                                            ...
                                            <div className="grid grid-cols-1 md:grid-cols-3 gap-4 pt-4 border-t border-gray-200">
                                                <div>
                                                    ...
                                                    <p className="text-sm font-medium text-gray-900">
                                                        {servicio.operario?.name || servicio.operario?.username || 'No asignado'}
                                                    </p>
                                                </div>
                                                <div>
                                                ...
                                                {rechazadoPorSupervisor && (
                                                    <div>
                                                        <p className="text-xs text-gray-500 mb-1">Rechazado por</p>
