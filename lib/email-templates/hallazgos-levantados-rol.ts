export type HallazgoLevantadoEmailItem = {
    seccion: string;
    itemNombre: string;
    observacion: string;
};

export type HallazgosLevantadosRolTemplateData = {
    ...
    hallazgos: HallazgoLevantadoEmailItem[];
};
...
export function buildHallazgosLevantadosRolEmail(
    data: HallazgosLevantadosRolTemplateData
): EmailTemplate {
    ...
    const htmlRows = data.hallazgos
        .map((hallazgo) => {
            return `
                <tr>
                    <td style="padding: 10px 12px; color: #111827; background: #ffffff; border: 1px solid #e5e7eb;">${escapeHtml(getSeccionLabel(hallazgo.seccion))}</td>
                    <td style="padding: 10px 12px; color: #111827; background: #ffffff; border: 1px solid #e5e7eb;">${escapeHtml(hallazgo.itemNombre)}</td>
                    <td style="padding: 10px 12px; color: #111827; background: #ffffff; border: 1px solid #e5e7eb;">${escapeHtml(hallazgo.observacion)}</td>
                </tr>
            `;
        })
        .join('');
    ...
    const textLines = [
        ...
        `Se detectaron ${data.hallazgos.length} hallazgos asignados a tu area (${rolLabel}).`,
        ...
        ...data.hallazgos.map((hallazgo, index) => {
            return `${index + 1}. [${getSeccionLabel(hallazgo.seccion)}] ${hallazgo.itemNombre} - Obs: ${hallazgo.observacion}`;
        }),
        '',
        'Ingresa a la plataforma para gestionar el cierre manual de estos hallazgos.',
    ]
        .filter((line): line is string => Boolean(line))
        .join('\n');

    return {
        subject: `Hallazgos detectados para ${rolLabel} - Servicio ${data.servicioCodigo}`,
        html: `
            <div style="font-family: Arial, sans-serif; color: #111827; line-height: 1.6; max-width: 760px; margin: 0 auto;">
                <div style="background: #1d4ed8; color: #ffffff; padding: 14px 18px; border-radius: 8px 8px 0 0;">
                    <h2 style="margin: 0; font-size: 20px;">Hallazgos detectados</h2>
                </div>
                <div style="border: 1px solid #d1d5db; border-top: 0; border-radius: 0 0 8px 8px; padding: 18px;">
                    <p style="margin: 0 0 12px;">Hola ${escapeHtml(destinatarioNombre)}, se detectaron <strong>${data.hallazgos.length}</strong> hallazgos para el area de <strong>${escapeHtml(rolLabel)}</strong>.</p>
                    ...
                    <table style="width: 100%; border-collapse: collapse; table-layout: fixed;">
                        <thead>
                            <tr>
                                <th style="text-align: left; padding: 10px 12px; background: #dbeafe; border: 1px solid #bfdbfe; color: #1e3a8a;">Seccion</th>
                                <th style="text-align: left; padding: 10px 12px; background: #dbeafe; border: 1px solid #bfdbfe; color: #1e3a8a;">Item</th>
                                <th style="text-align: left; padding: 10px 12px; background: #dbeafe; border: 1px solid #bfdbfe; color: #1e3a8a;">Observacion</th>
                            </tr>
                        </thead>
                        <tbody>${htmlRows}</tbody>
                    </table>
                    <p style="margin: 16px 0 0;">Gestiona el cierre manual en el modulo de hallazgos.</p>
                </div>
            </div>
        `,
        text: textLines,
